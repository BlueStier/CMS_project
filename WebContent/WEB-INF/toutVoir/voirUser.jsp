...
<body>
	<div class="container-fluid">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active">Voir tout les utilisateurs</li>
		</ol>
		<!-- Icon Cards-->

		<%
			MODEL<?> user = (MODEL<?>) session.getAttribute("user");
			DAO<?> u = DAOFactory.getTeamDAO();
			ArrayList<Team> liste = (ArrayList<Team>) u.get_all();
			int taille = liste.size();
			for (int i = 0; i < taille; i++) {
				if (liste.get(i).get_id() != user.get_id()){
		%>
				...
				<div class="card text-white bg-primary o-hidden h-100">
					...
					<div class="card-body">
						<div class="card-body-icon">
							<%
								if (liste.get(i).get_role().get_id() == 1) {
							%>
							<i class="fa fa-drupal"></i>
							<%
								}
									if (liste.get(i).get_role().get_id() == 3) {
							%>
							<i class="fa fa-comments"></i>
							<%
								}
									if (liste.get(i).get_role().get_id() == 2) {
							%>
						...
						<div class="mr-5"><%=liste.get(i).get_pseudo()%></div>
						<div class="mr-5">
							Rôle :
							<%=liste.get(i).get_role().get_nom()%></div>
					</div>
					...
					<div class="form-group">
						<div class="form-row">
							<div class="col-md-6">
								...
								<input type="hidden" name="modifUser"
									value="<%=liste.get(i).get_id()%>">
								<button class="btn btn-primary btn-block" name="confModifUser">Modifier</button>
							</div>
							<div class="col-md-6">
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-footer">
							...
							<input type="hidden" name="supUser"
								value="<%=liste.get(i).get_id()%>"></input>
							<button class="btn btn-primary" type="submit" name="confSupUser">Supprimer</button>
						</div>
					</div>
				</div>
			</div>
